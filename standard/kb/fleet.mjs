...
export function readWorktrees(root) {
  ...
  for (const block of out.split(/\n\n+/)) {
    ...
    const feature = branch.startsWith('feat/') ? branch.slice(5)
      ...
      : role === 'reviewer' ? 'review / verify station' : branch
    let ahead = ''
...
/* ------------------------------- Kickoff page ---------------------------- */
const TEMPLATES = {
  orchestrator: `You are the Orchestrator for this project, running on main ({{WORKTREE}}). Read ~/.claude/agents/orchestrator.md, VISION.md, docs/ROADMAP.md, and FLEET.md to orient. Run a fleet cycle: refresh FLEET.md and tell me the project state and what needs my decision. Do NOT start, dispatch, or merge anything unless I tell you to — recommend, don't act. Beyond that, act on whatever I direct. Think deeply — you hold the whole-project scope.`,
  planner: `You are the Planner for feature {{NAME}}. Brief: <one-line goal>. Read ~/.claude/agents/planner.md, VISION.md, docs/ROADMAP.md, docs/features/README.md, and any existing spec. Produce/extend docs/features/{{NAME}}.md with a deep numbered plan, leave a Progress log and a "→ Next: builder" handoff block, then STOP for my approval before any code. Think deeply — this plan drives everything downstream.`,
  builder: `You are a Builder Agent assigned to the worktree {{WORKTREE}} (branch {{BRANCH}}, feature {{FEATURE}}). Read ~/.claude/agents/builder.md and operate under it. Orient from VISION.md, your spec in docs/features/, and this worktree's HANDOFF.md (the "→ Next: builder" block). Build inside src/features/{{FEATURE}}/ only. Update HANDOFF.md (status + "→ Next" block) when you pause or finish.`,
  reviewer: `You are the Reviewer assigned to {{BRANCH}} (feature {{FEATURE}}), run from the review worktree. Read ~/.claude/agents/reviewer.md and verifier.md. Orient from that branch's HANDOFF.md "→ Next: reviewer" block, the spec, and git diff main..{{BRANCH}}. Review (read-only), then verify (tests + harness). Write findings to src/features/{{FEATURE}}/REVIEW.md and set the HANDOFF.md status + "→ Next" block. Think carefully.`,
  integrator: `You are the Integrator assigned to {{BRANCH}} (feature {{FEATURE}}), on main ({{WORKTREE}}). Read ~/.claude/agents/integrator.md. Confirm reviewed + verified (REVIEW.md / HANDOFF.md), then merge to main, apply INTEGRATION.md, wire it in, fold the changelog, flip ROADMAP to done, and verify the whole app. Stop and confirm before the final merge. Set status merged and run /fleet. Think carefully.`,
}
